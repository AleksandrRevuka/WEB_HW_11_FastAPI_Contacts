...
from src.schemas.addressbook import AddressbookResponse

router = APIRouter(prefix="/search", tags=["search_contacts"])


@router.get("/firstname/{contact_firstname}", response_model=List[AddressbookResponse])
...
@router.get("/lastname/{contact_lastname}", response_model=List[AddressbookResponse])
...
@router.get("/email/{contact_email}", response_model=List[AddressbookResponse])
